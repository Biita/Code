...
{	
 //   MCIMask0 |= (DATA_RX_INT_MASK) | (FIFO_INT_MASK);	    /* 仅使能接收中断 */
    MCIMask0 |= (DATA_RX_INT_MASK) | (MCI_RX_FIFO_EMPTY);	    /* 仅使能接收中断 */
	...
	MCIDataTimer = 0x0FFFFFFF;                       /* 读数据块超时值 */
...
INT8U SdHal_ReadBlock(sd_struct *sds, INT8U *recbuf)
{
...
#if LPC23xx_MCI_DMA_EN
    // DMA 控制器初始化
	DMA_Init(1, P2M, DMA_MCIFIFO, (INT32U)recbuf, DMA_SIZE);                                                           
	...
	do
	{                                                           /* 等待数据接收 */
	    if (lpc_mci->RxTxFlag == SD_NO_ERR)										  //----2011.6.14hena 问题所在，返回值为0xE0 SD_ERR_HARDWARE硬件出错
	        break;                                              /* 数据接收成功 */	
...
INT8U SdHal_BeforeWriteBlock(sd_struct *sds)
{
	...
	MCIMask0 = DATA_TX_INT_MASK | (MCI_TX_FIFO_FULL);	      /* 仅使能TX中断 */
...
INT8U SdHal_WriteBlock(sd_struct *sds, INT8U *sendbuf)
{
	...
	DMA_Init(0, M2P, (INT32U)sendbuf, DMA_MCIFIFO, DMA_SIZE);                                            /* DMA控制器初始化 */
...
********************************************************************************************************************/
INT32U DMA_Init(INT32U ChannelNum, INT32U DMAMode, INT32U SrcAddr, INT32U DstAddr, INT32U TransferSize)
{
	...
	if ( ChannelNum == 0 )
	{
		...
		if ( DMAMode == M2M )
		{
			...
			DMACC0DestAddr = DstAddr;
			/* Terminal Count Int enable */
			DMACC0Control = (TransferSize & 0x0FFF) | (0x04 << 12) | (0x04 << 15) 
		...
		else if ( DMAMode == M2P )
		{
			...
			/* Terminal Count Int enable */
			DMACC0Control = (TransferSize & 0x0FFF) | (0x01 << 12) | (0x02 << 15)
	...
	{   
		...
		if ( DMAMode == M2M )
		{
			...
			DMACC1SrcAddr = SrcAddr;
			DMACC1DestAddr = DstAddr;
			/* Terminal Count Int enable */
			DMACC1Control = (TransferSize & 0x0FFF) | (0x04 << 12) | (0x04 << 15) 
				| (0x02 << 18) | (0x02 << 21) | (1 << 26) | (1 << 27) | 0x80000000;	
		}
		else if ( DMAMode == M2P )
		{
		...
		else if ( DMAMode == P2M )
		{
			...
			/* The burst size is set to 8, the size is 8 bit too. */
			/* Terminal Count Int enable */
			DMACC1Control = (TransferSize & 0x0FFF) | (0x02 << 12) | (0x02 << 15)
				| (0x02 << 18) | (0x02 << 21) | (1 << 27) | 0x80000000;
		}
		else
		{
