...
//-------------------------------------------------------------------------
uint8 		BUFFER[SEC_Size];
uint8 		PB_RelativeSector=0;
uint16   	BPB_BytesPerSec;						//每个扇区字节数
uint8 		BPB_SecPerClus;							//每个簇的扇区数
uint16  	BPB_RsvdSecCnt;							//保留扇区数
uint8 		BPB_NumFATs;							//FAT 表的数目
uint16  	BPB_RootEntCnt;							//对于FAT32，此区域必须为0
uint16  	BPB_TotSec16;
uint16  	BPB_FATSz16;							//FAT占用的sectors
uint32  	BPB_HiddSec;
uint32  	BPB_TotSec32;
uint32  	BPB_FATsz32;							//每个FAT 占用的扇区数,针对FAT32
uint32  	BPB_RootClus;							//根目录所在第一簇的簇号
...
{					
//	MMC_Init();
//	if(MMC_read_sector(LBA,BUFFER)!=0)return SD_FAIL;
...
{
//	MMC_Init();
//	if(MMC_Write_sector(LBA, BUFFER)!=0)return SD_FAIL;
...
uint32  DirStartSec(void){							//获取根目录开始扇区号
	return BPB_RsvdSecCnt+BPB_NumFATs*BPB_FATsz32+(BPB_RootClus-2)*BPB_SecPerClus;
}
//-------------------------------------------------------------------------
...
uint16  GetEmptyDIR(void){							//获取根目录中可以使用的一项
	uint32  i,DirSecCut,DirStart,m,ID=0;			//2的16次方65536
	
	//DirSecCut=GetDirSecCount();
	DirSecCut=128;									//本系统允许最多1024个目录项扇区	2048		
...
uint8  GetFileID(uint8  Name[11],DIR *ID,uint16  *pIndex)
{
	...
	DirStart = DirStartSec();						//获取根目录开始扇区号
...
uint8  CreateFile(uint8  *Name,uint32  Size){	//创建一个空文件
	...
	EmptyBytes((uint8 *)&FileDir,sizeof(DIR));	                //内存清空
	...
	WriteDIR(GetEmptyDIR(),&FileDir);	   //写根目录
