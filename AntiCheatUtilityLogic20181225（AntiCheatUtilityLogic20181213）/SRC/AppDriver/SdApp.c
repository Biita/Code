...
/*******************************************************************************************************************
** 函数名称: ReadSDCardMultiBlock()				
**
** 功能描述: 从SD/MMC卡中读多个数据块
**
** 输　  入: INT32U blockaddr: 以块为单位的块地址, 例如, 卡开始的0 ~ 511字节为块地址0, 512 ~ 1023字节的块地址为1
			 INT32U blocknum : 接收的块数
**	         INT8U *recbuf   : 接收缓冲区首地址
** 输 　 出: 	
**
** 返 回 值: 0:   正确    >0:   错误码, 见 sddriver.h 文件
********************************************************************************************************************/
INT8U ReadSDCardMultiBlock(INT32U blockaddr, INT32U blocknum,INT8U *recbuf)
{
	return	SD_ReadMultiBlock(&sd_info, blockaddr, blocknum, recbuf);
}
...
/*******************************************************************************************************************
** 函数名称: WriteSDCardMultiBlock()				
**
** 功能描述: 向SD/MMC卡中写多个块	
**
** 输　  入: INT32U blockaddr: 以块为单位的块地址, 例如, 卡开始的0 ~ 511字节为块地址0, 512 ~ 1023字节的块地址为1
			 INT32U blocknum : 发送的块数
**           INT8U *sendbuf  : 发送缓冲区首地址	
**	         
** 输 　 出: 无
**
** 返 回 值: 0:   正确    >0:   错误码, 见 sddriver.h 文件
********************************************************************************************************************/
INT8U WriteSDCardMultiBlock(INT32U blockaddr, INT32U blocknum,INT8U *sendbuf)
{
	return	SD_WriteMultiBlock(&sd_info, blockaddr, blocknum,sendbuf);
}
