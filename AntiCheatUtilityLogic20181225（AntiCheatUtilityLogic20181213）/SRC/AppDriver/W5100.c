...
void W5100_Init(void)
{
	...
	Write_W5100(W5100_RCR,3);	  // 0x09C4-3-3.64s,wujinlong 2130910
...
uint8 Socket_Connect(SOCKET s)
{
	...
	//Write_W5100((W5100_S0_MR+s*0x100), S_MR_TCP | S_MR_MC );		/*设置socket为TCP模式 */  // 20121205
...
-----------------------------------------------------------------------------*/
uint8 S_tx_process(SOCKET s, uint32 size)
{
	unsigned int i,j;
...
LOOP:
	...
	if((i & IR_S0_INT) == IR_S0_INT)
	{
		...
		if(j&S_IR_DISCON)				/* 在TCP模式下Socket断开连接处理，自己添加代码 */
		{
			...
			Write_W5100(W5100_S0_CR, S_CR_OPEN);
			Write_W5100(W5100_S0_CR, S_CR_LISTEN);	
			S0_State=0;
		}
		...
		{
			Write_W5100(W5100_S0_CR, S_CR_CLOSE);		/* 关闭端口，等待重新打开连接 */  // 打开注释 wujinlong	20130910
			S0_State=0;
		}
	}
	...
	if((i&IR_S1_INT)==IR_S1_INT)
	{
		...
		{
			//Write_W5100(W5100_S1_CR, S_CR_LISTEN);
			Write_W5100(W5100_S1_CR, S_CR_CLOSE);		 // wujinlong 20131121
			Write_W5100(W5100_S1_CR, S_CR_OPEN);
			Write_W5100(W5100_S1_CR, S_CR_LISTEN);		
			S1_State=0;
		}
	...
	if((i&IR_S2_INT)==IR_S2_INT)
	{
		...
		{
			//Write_W5100(W5100_S2_CR, S_CR_LISTEN);
			Write_W5100(W5100_S2_CR, S_CR_CLOSE);		 // wujinlong 20131121
			Write_W5100(W5100_S2_CR, S_CR_OPEN);
			Write_W5100(W5100_S2_CR, S_CR_LISTEN);	
			S2_State=0;
		}
	...
	if((i&IR_S3_INT)==IR_S3_INT)
	{
		...
		if(j&S_IR_DISCON)		/* 在TCP模式下Socket3断开连接处理，自己添加代码 */
		{
			...
			Write_W5100(W5100_S3_CR, S_CR_CLOSE);		 // wujinlong 20131121
			Write_W5100(W5100_S3_CR, S_CR_OPEN);
			Write_W5100(W5100_S3_CR, S_CR_LISTEN);
			S3_State=0;
		}
