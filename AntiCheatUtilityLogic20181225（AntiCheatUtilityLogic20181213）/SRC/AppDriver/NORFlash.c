...
//2:	SST39VF1601C	新板
*/
#define		NORFLASHVER		1		

#if NORFLASHVER	== 1
#define	BYTE_5555	0x5555
#define	BYTE_2aaa	0x2aaa 
#define	BYTE_Sector	0x30
#define	BYTE_Block	0x50
#define	BYTE_Chip	0x10
#else
#if NORFLASHVER	== 2 
#define	BYTE_5555	0x555
#define	BYTE_2aaa	0x2aa 
#define	BYTE_Sector	0x50
#define	BYTE_Block	0x30
#define	BYTE_Chip	0x10
#else	//其他	 					   
#define	BYTE_5555	0x0
#define	BYTE_2aaa	0x0 
#define	BYTE_Sector	0x0
#define	BYTE_Block	0x0
#define	BYTE_Chip	0x0
#endif	//#if NORFLASHVER	== 2

#endif	//#if NORFLASHVER	== 1
...
uint8 EraseNORFlash(void)
{
    ...
    l_pu16Temp		= GetAddr(BYTE_5555);
    l_pu16Temp[0]	= 0xaaaa;						/* 第一个周期,地址0x5555,数据0xAA*/
    l_pu16Temp		= GetAddr(BYTE_2aaa);
    l_pu16Temp[0]	= 0x5555;						/* 第二个周期,地址0x2aaa,数据0x55*/
    l_pu16Temp		= GetAddr(BYTE_5555);
    l_pu16Temp[0]	= 0x8080;						/* 第三个周期,地址0x5555,数据0x80*/
    l_pu16Temp		= GetAddr(BYTE_5555);
    l_pu16Temp[0]	= 0xaaaa;						/* 第四个周期,地址0x5555,数据0xAA*/
    l_pu16Temp		= GetAddr(BYTE_2aaa);
    l_pu16Temp[0]	= 0x5555;						/* 第五个周期,地址0x2aaa,数据0x55*/
    l_pu16Temp		= GetAddr(BYTE_5555);
    l_pu16Temp[0]	= 0x1010;						/* 第六个周期,地址0x5555,数据0x10*/
...
uint8 EraseBlock(uint32 p_u32BlockID)
{
    ...
    l_pu16Temp[0]	= BYTE_Block;					/* 第六个周期,块地址,数据0x50  */
...
uint8 EraseSector(uint32 p_u32SectorAddr)
{
    ...
    l_pu16Temp[0]	= BYTE_Sector;					/* 第六个周期,块地址,数据0x30  */
    /* 
     *  等待操作完成 (若擦除操作没有完成，每次读操作DQ6会跳变) 
     */
    while (1)				
...
uint8 WordProgram(uint32 p_u32Address, uint16 p_u16Data)
{
	...
	l_pu16Temp[0]	= 0x5555;						/* 第二个周期,地址0x2aaa,数据0x55*/
	l_pu16Temp		= GetAddr(BYTE_5555);
	l_pu16Temp[0]	= 0xa0a0;						/* 第三个周期,地址0x5555,数据0x80*/
