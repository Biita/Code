...
void OSTaskCreateHook (OS_TCB *ptcb)
{
	...
	if(ptcb->OSTCBExtPtr)
		((TaskStatics *)ptcb->OSTCBExtPtr)->TaskID = ptcb->OSTCBPrio;
#else
...
#if	OS_TASK_CREATE_EXT_EN > 0
  	OS_STK_DATA data;
	if(OSTCBCur->OSTCBExtPtr)
	{
		((TaskStatics*)OSTCBCur->OSTCBExtPtr)->tmCnt = OSTimeGet() - ((TaskStatics*)OSTCBCur->OSTCBExtPtr)->tmCnt;	
		OSTaskStkChk(OS_PRIO_SELF,&data);
		((TaskStatics*)OSTCBCur->OSTCBExtPtr)->stkCurUsage = data.OSUsed;
		if(data.OSUsed > ((TaskStatics*)OSTCBCur->OSTCBExtPtr)->stkMaxUsage) 
			((TaskStatics*)OSTCBCur->OSTCBExtPtr)->stkMaxUsage = data.OSUsed;
//		((TaskStatics*)OSTCBCur->OSTCBExtPtr)->CPUsage  = ((TaskStatics*)OSTCBCur->OSTCBExtPtr)->tmCnt/OSIdleCtrMax;
		((TaskStatics*)OSTCBCur->OSTCBExtPtr)->CPUsage  = ((TaskStatics*)OSTCBCur->OSTCBExtPtr)->tmCnt;
//		((TaskStatics*)OSTCBHighRdy->OSTCBExtPtr)->tmCnt = 	OSTimeGet();
	}
#endif
}
